...
lambda = [200 200 200 200 500];         % ranking scaling factors
max_iter=10000;                  % stop criteria: maximum iterations
epsilon=10^-16;

% for both 's' and 'd' options
rTarg=4;                        % block-row index of the target matrix
cTarg=5;                        % column-row index of the target matrix

% for 's' option only
index_target = 0;               % index of the G target, i.e. object type we are interested in 

select = 'd';  %select = s: use the algorithm to find the relations between objects of the same type
